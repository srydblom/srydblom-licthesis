...
The research work flow is shown in \Cref{fig:res_flow}. RQ1-RQ4 can be answered by the work described in this thesis. To fully answer research question RQ5, a new measurement should be done by measuring LWC and MVD in combination with sensors capable of detecting ice on structures in real world conditions.

\tikzstyle{process} = [rectangle, draw, minimum width=10cm, text width=26em, rounded corners]
...
\node [process, yshift=-1cm] (pro1) {\small Investigate an optical setup for imaging droplets. Develop a method to calculate \gls{lwc} and \gls{mvd} from images. Develop a method to calibrate the measurement system. This step will answer research question \ref{itm:rq1} and is described in Paper I.};
\node [process, my below of=pro1] (pro2) {\small Develop an instrument that is capable of measuring LWC and MVD in field conditions. This step will answer research question RQ2 and is described in Paper II.};
\node [process, my below of=pro2] (pro3) {\small Develop a test environment that simulates real world conditions, preferrably where LWC and MVD can be controlled. This step will answer research question RQ3 and is described in Paper II.};
\node [process, my below of=pro3] (pro4) {\small Test the developed instrument in the real world. Use a reference instrument or method for evaluating the measured LWC and MVD. This step will answer research question RQ4 and is described in Paper III.};

\draw [arrow] (pro1) -- (pro2);
\draw [arrow] (pro2) -- (pro3);
\draw [arrow] (pro3) -- (pro4);
\end{tikzpicture}
\caption{The research work flow and the related research questions (RQ1 to RQ4).}
\label{fig:res_flow}
\end{figure}
%\hspace{-50pt}

Paper I describes a shadowgraph system that uses edge detection to define a measurement range depending on the size of a particle and the image \gls{snr}. Paper II describes the size measurement method, an evaluation of the instrument's  physical design with weather protection and a measurement of \gls{lwc} and the \gls{mvd} in a purpose-built fog chamber. Paper III describes a verification of the size measurement using polymer microspheres in calibrated distributions traceable from the \gls{nist} and a field study of \gls{lwc} and \gls{mvd} close to the ground using the \gls{dii} and a comparative study using the \gls{cdp}.
...
%The main scientific questions can be divided in following detailed questions. These questions are answered by the work in this thesis and in \Cref{sec:otherconclusions}.
%
%\begin{enumerate}
%\item Can the size measurement function be calibrated by using a micrometer dot scale and can we verify this by an independent sample of known size?
%\item Is the LED capable of emitting enough light power for this application?
%\item How do we make a weather proof casing for the described system?
%\item Is it possible to create a simple climate chamber that can be used for testin and use it to control the \gls{lwc} and the \gls{mvd}?
%\item What is the difference in \gls{lwc} measured by the \gls{cdp} and the \gls{dii} when considering that the \gls{cdp} can only measure particles smaller than 50 µm in diameter.
%\item What is the main difference in droplet sizes measured by the \gls{cdp} and the \gls{dii}.
%\item How can the measurement of \gls{lwc} with the \gls{dii} be improved?
%\item How do the measured \gls{lwc} and \gls{mvd} agree with the values predicted by the HARMONIE-AROME \gls{nwp}?
%\end{enumerate}
